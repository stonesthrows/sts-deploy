// ════════════════════════════════════════════
//  VENDORED  —  @block65/webcrypto-web-push v1.0.2 (MIT), bundled with its
//  two runtime deps (base64-arraybuffer, @block65/custom-error) into one
//  file via esbuild. Used only by functions/api/notion-messages.js for
//  VAPID signing + RFC 8291 payload encryption (Web Push notifications).
//
//  Why vendored instead of an npm dependency: this Cloudflare Pages
//  project has no configured build command, so `npm install` never runs
//  before the Functions bundler resolves imports — a real dependency here
//  fails the deploy. Vendoring keeps the whole site's deploy dependency-
//  free, matching every other file in functions/.
//
//  isomorphic-crypto's Node ("node:crypto") fallback branch was manually
//  stripped after bundling — Cloudflare Workers always provides
//  globalThis.crypto, and the dynamic import of a Node builtin was itself
//  a second build-resolution failure mode to avoid.
//
//  To regenerate after a version bump:
//    npm install @block65/webcrypto-web-push
//    npx esbuild node_modules/@block65/webcrypto-web-push/dist/lib/main.js \
//      --bundle --format=esm --platform=browser --target=es2022 \
//      --external:node:crypto --outfile=functions/_vendor/webcrypto-web-push.js
//  then re-apply the isomorphic-crypto trim above by hand.
// ════════════════════════════════════════════
...
// Trimmed from the upstream file: Cloudflare Workers always provides
// globalThis.crypto, so the Node "node:crypto" fallback (a dynamic import
// that Wrangler's bundler tries to resolve at deploy time and fails on,
// since this project has no nodejs_compat flag) is dropped entirely.
var impl = globalThis.crypto;
var crypto2 = {
  getRandomValues: (array) => impl.getRandomValues(array),
  subtle: impl.subtle
};
var CryptoKey2 = globalThis.CryptoKey;
