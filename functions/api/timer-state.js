...
export async function onRequestGet(context) {
  ...
  const [v0, v1] = await Promise.all([kv.get('timer_0'), kv.get('timer_1')]);
  return json({
...
export async function onRequestPut(context) {
  ...
  // TTL of 24 hours — stale timers auto-expire
  await kv.put(`timer_${tid}`, JSON.stringify(state), { expirationTtl: 86400 });
  return json({ ok: true });
}
