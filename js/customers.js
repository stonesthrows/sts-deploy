      ...
      <div class="ct-row-wrap" id="ct-wrap-${idx}">
        <div class="ct-row" onclick="toggleCustomerRow(${idx})">
          <div class="c-avatar-wrap">
            <div class="c-avatar">${esc(initials(c.name))}</div>
            <div>
...
function openCustomerDrawer(idx) {
  ...
  // Two escaping layers for values inside onclick attributes: backslash-escape
  // for the JS string literal, then esc() for the HTML attribute — the HTML
  // parser decodes entities before JS sees the string, so both are needed.
  ...
  dropdown.innerHTML = matches.map(c => {
    ...
    return `
      <div class="d-dropdown-item" onclick="openCustomerDrawer(${idx});">
        <div class="d-dd-avatar">${esc(initials(c.name))}</div>
        <div>
