...
//  Persistent Client Profile store + returning-client suggestions:
//  typing a name / email / phone on Step 1 surfaces matching past
//  customers in a banner, and "Load profile" fills their contact
//  details in. Nothing fills automatically — only an explicit tap.
//
//  Store: localStorage['sts-customers'] = { v, updated, profiles: [...] }
//  Profile: { id, name, email, phone, address{}, orderIds[],
//             orderCount, lastOrderAt, lastOrderLabel, updatedAt }
//
//  Scope note: this file once also carried the ring-size registry,
//  occasion/gift and style-profile sections of Step 1. Those were
//  removed with the rest of the intake simplification; profiles saved
//  while they existed may still carry ringSizes/styleProfile/occasions
//  keys, which this file leaves untouched rather than deleting.
//
...
//  is unsafe outside the main app). `updatedAt` is kept per profile so
//  a future push to a Notion "Customers" database can diff.
// ════════════════════════════════════════════
...
function stsCustUpsertFromOrder(order, db) {
  ...
  if (!p) {
    p = { id: 'c' + Date.now() + Math.random().toString(36).slice(2, 6),
          name: '', email: '', phone: '', address: null, orderIds: [],
...
// Only the fields "Load profile" writes — so Undo restores exactly
// what the fill touched and nothing else.
function _profFieldIds() {
  return ['f-email', 'f-phone', 'f-addr-street', 'f-addr-street2',
          'f-addr-city', 'f-addr-state', 'f-addr-zip', 'f-addr-country'];
}
